...
async function send(){
  ...
  const fingerprint=`${text}||${(S.pendingFiles||[]).map(f=>f&&f.name?f.name:String(f)).join('|')}`;
  const now=Date.now();
  if(fingerprint&&fingerprint===_lastSendFingerprint&&(now-_lastSendAt)<1500){
    console.warn('send(): duplicate submission suppressed');
    return;
  }
  _sendLock=true;
  ...
  if(text.startsWith('/')&&!S.pendingFiles.length&&executeCommand(text)){
    $('msg').value='';autoResize();hideCmdDropdown();_sendLock=false;return;
  }
  // Don't send while an inline message edit is active
  if(document.querySelector('.msg-edit-area')){_sendLock=false;return;}
  ...
  setStatus(S.pendingFiles&&S.pendingFiles.length?'업로드 중…':'전송 중…');
  let uploaded=[];
  try{uploaded=await uploadPendingFiles();}
  catch(e){if(!text){setStatus(`❌ ${e.message}`);_sendLock=false;return;}}

  let msgText=text;
  if(uploaded.length&&!msgText)msgText=`I've uploaded ${uploaded.length} file(s): ${uploaded.join(', ')}`;
  else if(uploaded.length)msgText=`${text}\n\n[Attached files: ${uploaded.join(', ')}]`;
  if(!msgText){setStatus('보낼 내용이 없습니다');_sendLock=false;return;}
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('compressed',e=>{
      ...
      try{
        ...
        showToast(d.message||'컨텍스트를 압축했습니다');
      }catch(err){}
    });
    ...
    source.addEventListener('cancel',e=>{
      ...
      if(S.session&&S.session.session_id===activeSid){
        clearLiveToolCards();if(!assistantText)removeThinking();
        S.messages.push({role:'assistant',content:'*작업이 취소되었습니다.*'});renderMessages();
      }
      renderSessionList();
      if(!S.session||!INFLIGHT[S.session.session_id]){setBusy(false);setStatus('');}
    });
  ...
  function _handleStreamError(){
    ...
    if(S.session&&S.session.session_id===activeSid){
      S.activeStreamId=null;const _cbe=$('btnCancel');if(_cbe)_cbe.style.display='none';
      clearLiveToolCards();if(!assistantText)removeThinking();
      S.messages.push({role:'assistant',content:'**오류:** 연결이 끊어졌습니다'});renderMessages();
    }else{
    ...
    if(!S.session||!INFLIGHT[S.session.session_id]){setBusy(false);setStatus('오류: 연결이 끊어졌습니다');}
  }
...
function transcript(){
  const lines=[`# Hermes session ${S.session?.session_id||''}`,``,
    `작업공간: ${S.session?.workspace||''}`,`모델: ${S.session?.model||''}`,``];
...
async function respondApproval(choice) {
  ...
  } catch(e) { setStatus("승인 처리 오류: " + e.message); }
}
