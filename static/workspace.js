...
async function loadDir(path){
  ...
  try{
    ...
    const we=$('workspaceEmpty'); if(we) we.style.display='none';
    ...
    if(typeof clearPreview==='function'){
      if(typeof _previewDirty!=='undefined'&&_previewDirty){
        if(confirm('미리보기에 저장되지 않은 변경 사항이 있습니다. 버리고 이동할까요?'))clearPreview();
      }else{
        clearPreview();
      }
    }
...
function updateEditBtn(){
  ...
  btn.innerHTML = editing ? '&#128190; 저장' : '&#9998; 편집';
  btn.title = editing ? '변경 사항 저장' : '이 파일 편집';
  btn.style.color = editing ? 'var(--blue)' : '';
  if(_previewDirty) btn.innerHTML = '&#128190; 저장*';
}
...
async function openFile(path){
  ...
  if(IMAGE_EXTS.has(ext)){
    ...
    $('previewImg').alt=path;
    $('previewImg').src=url;
    $('previewImg').onerror=()=>setStatus('이미지를 불러오지 못했습니다');
    ...
    }catch(e){setStatus('파일을 열지 못했습니다');}
  } else {
