...
function openMemoryWrite() {
  switchPanel('memory');
  // If not already in edit mode, toggle it
...
function renderWorkspacesPanel(workspaces){
  ...
  addRow.innerHTML=`
    <input id="wsAddInput" placeholder="작업공간 경로 추가 (예: /home/user/my-project)" style="flex:1;background:rgba(255,255,255,.06);border:1px solid var(--border2);border-radius:7px;color:var(--text);padding:7px 10px;font-size:12px;outline:none;">
    <button class="ws-action-btn" onclick="addWorkspace()">&#43; 추가</button>`;
  panel.appendChild(addRow);
  const hint=document.createElement('div');
  hint.style.cssText='font-size:11px;color:var(--muted);padding:4px 0 8px';
  hint.textContent='저장 전에 실제 존재하는 디렉터리인지 확인합니다.';
...
async function addWorkspace(){
  ...
  try{
    ...
    showToast('작업공간을 추가했습니다');
  }catch(e){setStatus('추가 실패: '+e.message);}
}
