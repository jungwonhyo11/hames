...
_FALLBACK_MODELS = [
    ...
    {'provider': 'Other',     'id': 'ganma4',                           'label': 'Ganma 4'},
]
...
def resolve_model_provider(model_id: str) -> tuple:
    ...
    bare model names, while some cross-provider picks should route through a
    provider the user has actually authenticated.
    ...
    # Load auth store so WebUI can route to authenticated providers even when
    # config.yaml is pinned to a local/custom model such as Darwin.
    auth_store = {}
    try:
    ...
    # absolute local model paths like /Users/.../Darwin-35B-....
    if model_id.startswith('/') or model_basename.startswith(local_family_prefixes) or model_id.startswith(local_family_prefixes):
        if 'custom:darwin' in auth_providers:
    ...
    if '/' not in model_id and config_provider and str(config_provider).startswith('custom:'):
        if model_id in openai_family | codex_family:
            if 'openai-codex' in auth_providers:
                return model_id, 'openai-codex', None
            if 'openai' in auth_providers:
                return model_id, 'openai', None
    ...
    if '/' in model_id:
        ...
        # Explicit OpenAI-family pick: prefer authenticated direct provider over
        # OpenRouter when available.
        if prefix == 'openai':
            if 'openai' in auth_providers:
                return bare, 'openai', None
            if 'openai-codex' in auth_providers:
...
def get_available_models() -> dict:
    ...
    # 3. Try to read auth store for active provider and additional authenticated
    # providers (if hermes is installed). Even when config.yaml pins a local
    # default provider like custom:darwin, the dropdown should still surface
    # other providers the user has already authenticated (e.g. openai-codex).
    auth_store = {}
    try:
        from api.profiles import get_active_hermes_home as _gah
        auth_store_path = _gah() / 'auth.json'
    ...
    # Add providers present in Hermes auth store / credential pool so WebUI can
    # offer all authenticated backends, not just the provider pinned in config.
    for pid in (auth_store.get('providers') or {}).keys():
        if pid:
            detected_providers.add(str(pid))
    for pid in (auth_store.get('credential_pool') or {}).keys():
        if pid:
            pid = str(pid)
            # Avoid cluttering the dropdown with every custom local endpoint from
            # the credential pool. Keep the currently active custom provider, but
            # surface authenticated non-custom providers like openai-codex.
            if pid.startswith('custom:') and pid != active_provider:
                continue
            detected_providers.add(pid)
            # ChatGPT/Codex OAuth can usually serve standard OpenAI-family model
            # IDs too, so expose the OpenAI group alongside Codex.
            if pid == 'openai-codex':
